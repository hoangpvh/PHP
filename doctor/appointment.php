            ...
            <div class="row">
                ...
                <tbody>
                  <?php 
                    ...
                    $id = $_SESSION['doctor'];
                    $query = "select * from appointment where doc_id='$id' order by appdate desc;";
                    ...
                    if(($row['userStatus']==1) && ($row['doctorStatus']==0))  
                    {
                     ...
                     <td>
                        ...
                        <a href="prescribe.php?doc_id=<?php echo $row['doc_id']?>&appdate=<?php echo $row['appdate']?>&patient_id=<?php echo $row['patient_id']?>"
                        tooltip-placement="top" tooltip="Remove" title="prescribe">
                        <button class="btn btn-info" style="color: white;">Prescribe</button></a>
                        <?php } if(($row['userStatus']==0) || ($row['doctorStatus']==0)) {

                            echo "-";
                            } if(($row['userStatus']==1) && ($row['doctorStatus']==1)&&($row['status'] == 1)){ ?> 
                              <a href="view.php?doc_id=<?php echo $row['doc_id']?>&appdate=<?php echo $row['appdate']?>"><button class="btn btn-success">View</button></a>
                            <?php }?>
